...
Windows uses the bash launcher, so Git for Windows must be installed.
...
The plugin payload — the skill, `VERSION.txt` and `checksums.txt` — is generated by
`release.sh` in the review raven source repository, which builds the binaries in CI and
attaches them to a release here. `VERSION.txt` names the version those binaries were
built from.
